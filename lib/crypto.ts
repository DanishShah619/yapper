 ...
 */
export function savePrivateKey(userId: string, privateKeyJwk: string): void {
...
export async function generateRoomKey(): Promise<CryptoKey> {
  return crypto.subtle.generateKey(
    ...
    true,           // extractable — needed to wrap it for each member
    ["encrypt", "decrypt"]
  );
}
...
export async function wrapRoomKeyForMember(
  ...
  const senderPrivateKey = await crypto.subtle.importKey(
    "jwk",
    JSON.parse(senderPrivateKeyJwk),
    { name: "ECDH", namedCurve: "P-256" },
    false,
    ["deriveKey"]
  );
  ...
  const sharedKey = await crypto.subtle.deriveKey(
    ...
    senderPrivateKey,
    { name: "AES-KW", length: 256 },  // AES Key Wrap — designed specifically for wrapping keys
    false,
    ["wrapKey"]
  );
...
export async function unwrapRoomKey(
  ...
  const wrappedKeyBuffer = Uint8Array.from(
    atob(encryptedShard), c => c.charCodeAt(0)
  ).buffer;
